...
internal class ContainConstrain<T>(val element: T, val container: Iterable<T>) : Constrain<T>() {
...
internal class RangeConstrain<T : Comparable<T>>(val bigger: Range<T>, val actual: Range<T>) :
    Constrain<ClosedRange<T>>() {

    override fun test(): Boolean =
        bigger.start <= actual.start && bigger.endInclusive >= actual.endInclusive
}

internal class EqualConstrain<T : Comparable<T>>(val expected: T, val actual: T) : Constrain<T>() {
    override fun test(): Boolean = expected == actual
}

internal class ConstrainsWrapper<T>(val constrains: List<AnyConstrain>, val tests: List<Predicate<T>>)
// may be not as a constrain?
